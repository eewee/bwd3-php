<img src="./images/visuel-php.jpg" class="img-fluid" alt="Visuel PHP">

<h1>PHP initiation</h1>
<ul>
    <li><a href="./etape01.php">Variable</a></li>
    <li><a href="./etape02.php">Condition</a></li>
    <li><a href="./etape03.php">Boucle</a></li>
    <li><a href="./etape04.php">Fonction</a></li>
    <li><a href="./etape05.php">$_GET, $_POST, $_SESSION</a></li>
    <li><a href="./etape06.php">Base de données</a></li>
</ul>